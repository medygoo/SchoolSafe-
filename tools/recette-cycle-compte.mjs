...
new Function(extraire('window._COMPTE_REFUS = {', "window._parQ = ''"))();
...
ok('l’étape 1 prépare le retrait',
   noms.includes('prepare_school_account_removal'), noms.join(','));
ok('l’étape 2 retire l’identité de connexion, hors du navigateur',
   noms.includes('remove-school-account'), '');
ok('l’étape 3 CONFIRME auprès du serveur',
   noms.includes('confirm_school_account_removal'), '');
ok('et dans cet ordre',
   noms.indexOf('prepare_school_account_removal') < noms.indexOf('remove-school-account')
   && noms.indexOf('remove-school-account') < noms.indexOf('confirm_school_account_removal'), noms.join(','));
