...
-- 2. Index d'unicité mensuelle / obligation
SELECT
  'indexes' AS check_group,
  bool_and(indexname IS NOT NULL) AS ok,
  array_agg(indexname ORDER BY indexname) AS found
FROM (
  SELECT wanted.indexname, p.indexname AS actual
  FROM (VALUES
  ...
  LEFT JOIN pg_indexes p
    ON p.schemaname='public' AND p.indexname=wanted.indexname
) x(indexname,actual);

-- Version plus explicite si l'outil client n'affiche pas bool_and comme prévu.
SELECT wanted.indexname,
       EXISTS(SELECT 1 FROM pg_indexes p WHERE p.schemaname='public' AND p.indexname=wanted.indexname) AS exists
FROM (VALUES
  ('uq_rattrapages_auto_sid_subject_month'),
  ('uq_convocations_auto_rattrapage_sid_month'),
  ('uq_rattrapages_fee_obligation')
...
-- 3. RPC publiques attendues : signatures exactes
SELECT
  p.proname,
...
-- 4. Fonction interne de détection : service_role uniquement pour l'automatisation.
SELECT
  p.proname,
...
-- 5. Le registre rattrapages ne doit plus être directement mutable/lisible
...
-- 6. Le paiement général doit ignorer les obligations manual-only en auto-allocation.
SELECT
...
-- 7. Réconciliation paiement / contrepassation
...
-- 8. Notifications : la déduplication et la file Push doivent déjà exister.
SELECT
...
-- 9. Aucun test ci-dessus ne révèle montant réel, nom d'enfant, token Push ou secret.
